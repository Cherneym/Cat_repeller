...
#############This section initializes HAT Relays for RPi5################
import gpiod
import subprocess
...
img_height = 320
##############################################################################
##################Joe said I can eliminate one of the variables below, cant' remember which one######################
latch_time=0
def set_latch_time():
    global latch_time
    latch_time = time.time()
    return latch_time
def set_latch_timediff():
    global latch_timediff
    latch_timediff = time.time() - latch_time
    return latch_timediff
latch_duration = 5 #Duration for which the latch should hold in seconds

is_latched = False #state of the latch
def set_is_latched_false():
    global is_latched
    is_latched = False
    return is_latched

def set_is_latched_true():
    global is_latched
    is_latched = True
    return is_latched

#current_time = time.time()  #get current time  #current_time does not update if not located within a loop
#times_up = 0	#threshold exceeded, release the latch

##################################################################################
#######################This section gets date and human clock time from Suntime API###################
...
class LetterBox:
    def __init__(
        ...
        self.scaleup = scaleup#alt 3 comments out code, alt 4 uncomments out code
        self.stride = stride
...
#                subprocess.run(['pinctrl', 'set', '18', 'op', 'dl'], check=True)   #Turn pin 18 (trigger) off
...
#            set_latch_timediff()    #Call Global function
            #            print('current time = ', current_time)
            #            print('latch time = ', latch_time)
#            print('latch time diff = ', latch_time_diff)
#            print('latched state ', is_latched)

############################## Release latch ################################################            
        if is_latched and (current_time - latch_time >=latch_duration):
           set_is_latched_false() #Call Global function to rest the latch
           print("Latch released")
#               subprocess.run(['pinctrl', 'set', '4', 'op', 'dl'], check=True)   #Turn pin 4 off
#               subprocess.run(['pinctrl', 'set', '18', 'op', 'dl'], check=True)   #Turn pin 18 (trigger) off 
#               print('Human detected, relay and USS turned off after 5 secs')
#           break 
############################## Turn off relay and ultrasonsics ################################################            
#        if is_latched == False:
           subprocess.run(['pinctrl', 'set', '22', 'op', 'dl'], check=True)   #Turn pin 22 off
           subprocess.run(['pinctrl', 'set', '18', 'op', 'dl'], check=True)   #Turn pin 18 (trigger) off 
           print('Human detected, light and USS turned off after 5 secs')
# subprocess.run(['pinctrl', 'set', '4', 'op', 'dl'], check=True)   #Turn pin 4 off
# subprocess.run(['pinctrl', 'set', '18', 'op', 'dl'], check=True)   #Turn pin 18 (trigger) off
# subprocess.run(['pinctrl', 'set', '22', 'op', 'dl'], check=True)   #Turn pin 22 off
